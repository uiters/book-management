...
const Users = () => {
    ...
    const [data, setData] = useState<UsersViewModel[]>([]);
    ...
    const getUsersPage = () => {
      usersApi
        .getPagedUsers(searchKey, searchTitle, page, countPerPage)
        .then((response) => {
          ...
          usersApi
            ...
            .then((res) => {
              ...
              if (res.status === 200) {
                ...
                const removedList = data.filter((users: UsersViewModel) => users.id !== id);
                setData(removedList);
    ...
    return (
      ...
      <Fragment>
        <div className="related-items items-start border mt-8 gap-y-5 bg-white">
          <br />
          <h1 className="mb-10 font-bold text-2xl float-center">USERS PAGE</h1>
          <Link to={PATHS.NEWUSERS} className="py-3 px-6 bg-blue-400 hover:bg-blue-600 rounded-md text-white text-sm mt-8" >Add New</Link>
            ...
            <div className="mt-7 mb-3 ml-56 mr-28 flex" style={{ justifyContent: 'between' }}>
              <div className="select mr-2 border-2 border-gray-200 float-right w-1/5">
                <select 
                    ...
                    className = "p-2 rounded-xl w-11/12 outline-none"
                     >
              ...
              <div className="field mr-2 w-3/5">
                <div className="control">
                  <input
                    type="text"
                    className="input border-2 border-gray-200 rounded-xl p-2 float-left w-full outline-none"
            ...
            <div className="flex flex-col p-3">
              ...
              <table className='border-collapse border border-blue-800 -mt-3 p-3 justify-center'>
                <thead className='bg-blue-500'>
                  <tr>
                    <th className="border border-blue-600 text-left p-3 w-4/12">Username</th>
                    <th className="border border-blue-600 text-left p-3 w-4/12">Email</th>
                    <th className="border border-blue-600 text-left p-3 w-3/12">Role</th>
                ...
                <tbody>
                  ...
                  {totalCount > 0 && data.map((item: any) => (
                    <tr className="hover:bg-gray-200" key={item.id}>
                      <td className="border border-blue-600 text-left p-3">{item.username}</td>
                      <td className="border border-blue-600 text-left p-3">{item.email}</td>
                      <td className="border border-blue-600 text-left p-3">{item.role}</td>
                      <td className="border border-blue-600">
                        <Link to={"/users/update/" + item.id} className="badge badge-warning float-left ml-3 ">
